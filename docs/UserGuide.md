...
| Phone includes unusual symbols/format/spaces without any comma | `⚠ Warning: Phone number may contain multiple numbers. Use "," to separate them if intended.`                                                              | Phone is accepted, but [format may be unintended](#contact-phone-format). You can safely ignore it if you're providing labels eg. `61234567 (Office)`                                    |
...
* Multiple phone numbers should be separated by commas so that duplicate phone numbers across contacts can be detected correctly.
