...
| Email may be using a non-standard domain format                 | `⚠ Warning: Email may be missing parts of a typical domain (e.g. "."), is this intentional?`                                                                    | Email is accepted, but uses a valid non-standard domain format (e.g. `user@localhost`, without a “.”).  You can verify if the email entered is correct.        |
